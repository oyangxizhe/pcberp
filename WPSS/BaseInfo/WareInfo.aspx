...
<span  class ="c13102102"><asp:Label ID="prompt" runat="server"  ForeColor="#f80707"></asp:Label></span>
     ...
     '<%#Eval ("CWAREID") %>','<%#Eval ("CNAME") %>','<%#Eval ("SELLUNITPRICE") %>','<%#Eval ("SPEC") %>','<%#Eval ("REMARK") %>'
             ,'<%#Eval ("Set长") %>','<%#Eval ("Set宽") %>','<%#Eval ("Set排版数") %>','<%#Eval ("MaxStorageName") %>','<%#Eval ("MaxStorageBatchID") %>',
             '<%#Eval ("MaxStorageCount") %>', '<%#Eval ("PURCHASEUNITPRICE") %>')">
                       选取</a>
                   </ItemTemplate>
...
<script type="text/javascript" language="javascript">
    ...
    function f13100302(obj, obj1, obj2, obj3, obj4, obj5, obj6, obj7, obj8,obj9,obj10,obj11,obj12,obj13,obj14) {
        ...
        if (navigator.userAgent.indexOf("Chrome") > 0) {
            ...
            if (c1 == "purchaseunitprice") {
            ...
            else if (c1 == "purchase") {//来自采购单请求品号信息
                ...
                for (i = 1; i < tr.length; i++) {
                    if (objx == i) {
                        ...
                        tr[i].getElementsByTagName("td")[5].getElementsByTagName("input")[0].value = obj14;//单价
                        break;
                    }
                }
            }
